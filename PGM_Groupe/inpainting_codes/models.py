...
class SimpleScoreNetwork(nn.Module):
    def __init__(self, hidden_dim: int = 128, data_dim: int = 2) -> None:
        ...
        # Define layers, adjust input size to accommodate conditional labels
        self.layers = nn.Sequential(
            nn.Linear(data_dim + 1, hidden_dim),  # Adjust the input size to accommodate labels
            nn.Softplus(),
    ...
    def forward(self, input: torch.Tensor, labels: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the model. If `labels` are provided (for conditional models),
        they are concatenated to the input tensor `x`.
        """
        if labels is not None:
            # Concatenate labels to the input (typically along the feature dimension)
            input = torch.cat([input, labels.view(-1, 1)], dim=1)  # Assuming labels are 1D and need reshaping
    ...
    def sample(
        ...
        with torch.no_grad():
            for k in range(n_steps):
                score = self(x_k)  # This calls the forward method
